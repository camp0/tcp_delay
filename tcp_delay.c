...
void tcp_delay_timer_callback(unsigned long data)
{
	...
	if (sk) {
		...
		pr_info("tcp_delay: timer expires on connection (%s)\n", node->key);

		ret = del_timer(&node->timer);
		if (ret)
			pr_info("tcp_delay: The timer is still in use for connection (%s)\n", node->key);

	} else
		/* The connection dont exists on the kernel, timeout or rst/fin */
		pr_info("tcp_delay: timer expires but connection (%s) not found\n", node->key);
...
int tcp_delay_process_connection(char *local_ip, char *remote_ip,
	uint16_t local_port, uint16_t remote_port, char *key_str)
{
	struct sock *sk = NULL;
	...
	hash_for_each_possible_rcu(tcp_delay_hash_table, entry, hash_node, hash)
		if (entry->ipsrc == ipsrc && entry->ipdst == ipdst &&
			entry->portsrc == local_port && entry->portdst == remote_port)
			break;
	...
	if (sk == NULL) {
		pr_debug("tcp_delay: connection (%s) not found(%d)(%d)\n", key_str, local_port, remote_port);
		return -1;
	}
	...
	if (entry == NULL) {
		...
		pr_debug("tcp_delay: adding new connection (%s)\n", key_str);
	...
	if (ret)
		pr_debug("tcp_delay: can not modify timer\n");

	pr_info("tcp_delay: connection (%s) timer setup\n", key_str);
...
static int parse_user_input(char *buffer, int length)
{
	...
	char port_buffer[MAX_PORT_BUFFER];
	int local_port = 0;
	int remote_port = 0;
	int prev_idx = 0;
	int curr_idx = 0;
	...
	memset(&port_buffer, 0, MAX_PORT_BUFFER);
...
{
	pr_info("TCPDelay v1.0 registered\n");
	...
	if (proc_file_entry == NULL) {
		pr_err("tcp_delay: unable to register proc file\n");
		return -ENOMEM;
	}
	return 0;
}
...
static void __exit tcp_delay_exit(void)
{
	...
	pr_info("TCPDelay v1.0 unregistered\n");
